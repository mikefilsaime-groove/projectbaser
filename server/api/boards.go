...
func (a *API) handleDuplicateBoard(w http.ResponseWriter, r *http.Request) {
        ...
        if board.IsTemplate && board.Type == model.BoardTypeOpen {
                // Multi-tenant: Users can only access templates in their own team or global templates (team_id="0")
                if board.TeamID != model.GlobalTeamID {
                        userTeamID := getTeamID(r)
                        if userTeamID != board.TeamID && !a.permissions.HasPermissionToTeam(userID, board.TeamID, model.PermissionViewTeam) {
                                a.errorResponse(w, r, model.NewErrPermission("access denied to board"))
                                return
                        }
                }
